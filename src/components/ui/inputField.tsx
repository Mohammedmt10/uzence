...
export default function InputField(props : InputFieldProps) {
    ...
    const mode = props.darkMode ? "bg-black text-white placeholder:text-white" : ""
    ...
    return <div className={`${mode}`}>
        <form>
            ...
            {props.invalid && <div className="text-xs text-red-500 border-1 w-fit p-1 rounded mb-2">{props.errorMessage}</div>}
            
            <button className={`${!props.darkMode ? "border-white border-1" : ""} bg-black text-white border-1 border-black hover:bg-white hover:text-black duration-300 rounded-sm px-1 cursor-pointer block mt-2`} type="reset">Clear</button>
        </form>
    </div>
}